...
async function autoOAuthLogin(domain) {
  ...
  // 1. 获取 linuxdo_client_id
  let clientId;
  try {
    const resp = await fetch(`https://${domain}/api/status`);
    const data = await resp.json();
    clientId = data.data?.linuxdo_client_id || data.linuxdo_client_id;
    if (!clientId) {
  ...
  // 2.5. 获取 OAuth state (CSRF 保护)
  let state;
  try {
    const stateResp = await fetch(`https://${domain}/api/oauth/state`, {
      credentials: 'include'
    });
    ...
    if (stateData.success && stateData.data) {
      ...
      console.warn('[OAuth] 获取 state 失败:', stateData);
      return null;
    }
  } catch (e) {
  ...
  let tab;
  try {
    tab = await chrome.tabs.create({ url: oauthUrl, active: false });
    console.log(`[OAuth] 创建后台标签页 ${tab.id}`);
  } catch (e) {
    console.error('[OAuth] 创建标签页失败:', e);
    return null;
  }
  ...
  try {
    ...
    // 8. 导航到首页以捕获认证头（session 已在 OAuth 回调页面建立）
    console.log('[OAuth] 导航到首页以捕获认证头...');
